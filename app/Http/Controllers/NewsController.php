...
class NewsController extends Controller
{
    ...
    public function __construct(NewsRequest $request, News $model)
    {
    ...
    public function index()
    {
        ...
        if (is_null($this->request->value)) {//
            ...
            $this->request->flash();
            $rows = $this->model->where('title_en', 'like', "%{$this->request->value}%")
                ->paginate(25);
        }
...
//Insert Blog Gallary
            ...
            if ($this->request->ajax())
                return response()->json(array('status' => 'true', 'message' => "Add news Done Sucessfully"));
            return redirect()->back()->with('success', "Add news Done Sucessfully");
