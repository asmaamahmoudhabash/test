...
class TourController extends Controller
{
    ...
    public function __construct(TourRequest $request, Tour $model)
    {
    ...
    public function index()
    {
        ...
        if (is_null($this->request->value)) {
            ...
            $this->request->flash();
            $rows = $this->model->where('place_en', 'like', "%{$this->request->value}%")
                ->paginate(25);
        }
...
//           return $rows;
    ...
    {
//        $this->request->merge(['slug' => $this->request->place_en]);
       
//        $insert = $this->model->create($this->request->all());


        $this->request->merge(['vedio' => str_replace("watch?v=","embed/",$this->request->vedio) ]);
